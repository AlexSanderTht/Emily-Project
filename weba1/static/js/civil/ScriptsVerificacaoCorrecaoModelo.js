...
var NomeEtapa = "Verificação da Correção de Modelo";
...
function AlterarValorLVE(Elemento, ID, valor, NomeBotao, Funcao)
{
            ...
            if(valor == "Х")
            {
                ...
                $("#TituloAV"+NomeBotao).prop('title', `Verificação de Correção - Com Erros`);
                $("#TituloAV"+NomeBotao).addClass( "VCERRO" )
            }
            else if(valor == "✓")
            {
                ...
                $("#BotaoNOK"+NomeBotao).css("background-color", "#6c757d66");
                 $("#TituloAV"+NomeBotao).prop('title', `Verificação de Correção - OK`);
                  $("#TituloAV"+NomeBotao).removeClass( "VCERRO" )
            }
            else
            {
                ...
                $("#BotaoOK"+NomeBotao).css("background-color", "#6c757d66");
                $("#BotaoNOK"+NomeBotao).css("background-color", "#6c757d66");
                 $("#TituloAV"+NomeBotao).prop('title', `Verificação de Correção - Não se Aplica`);
                     $("#TituloAV"+NomeBotao).removeClass( "VCERRO" )

            }
CheckBotaoAddVerif();
}
function CheckBotaoAddVerif()
{
      var ItensComErro = $(".VCERRO").map(function() {
                return this;
            }).get();
            if(ItensComErro.length > 0)
            {
                $("#ConHeight").height("50px")
                $("#CriarNvVerifErr").show()

            }
            else
            {
                $("#ConHeight").height("0px")
                $("#CriarNvVerifErr").hide()
                    ...
                    var HTMLHeader = "";
                        ...
                        /*    var el = jQuery(`
                         ...
                         var HTMLHeader = "";
                            ...
                            {
                                   var el = jQuery(`<div class="tituloAV"  style="background-color: #6c757d66" title="Correção - Não Preenchido"><center>C<center></div>`, {
                                 }).appendTo('#GridBts'+i+j+k);
                            ...
                            {
                                   var el = jQuery(`<div class="tituloAV"  style=" background-color: #28a74566" title="Correção - OK"><center>C<center></div>`, {
                                    }).appendTo('#GridBts'+i+j+k);
                            ...
                            {
                                    var el = jQuery(`<div class="tituloAV" style=" background-color: #c8233366" title="Correção - Com Erros"><center>C<center></div>`, {
                                    }).appendTo('#GridBts'+i+j+k);

                            }
                            else
                            {
                                   var el = jQuery(`<div class="tituloAV"  style=" background-color: #6c757d66" title="Verificação - Não se Aplica"><center>C<center></div>`, {
                                }).appendTo('#GridBts'+i+j+k);
                            ...
                            if(ItemsSelecionados[k].ValorVerificacaoCorrecao == "" || !ItemsSelecionados[k].ValorVerificacaoCorrecao)
                            {
                                   var el = jQuery(`<div class="tituloAV" id="TituloAV${i}${j}${k}" style="background-color: #6c757d66" title="Verificação de Correção - Não Preenchido"><center>VC<center></div>`, {
                                 }).appendTo('#GridBts'+i+j+k);

                            }
                            else
                            if(ItemsSelecionados[k].ValorVerificacaoCorrecao == "✓")
                            {
                                   var el = jQuery(`<div class="tituloAV" id="TituloAV${i}${j}${k}" style=" background-color: #28a74566" title="Verificação de Correção - OK"><center>VC<center></div>`, {
                                    }).appendTo('#GridBts'+i+j+k);
                                       CorBotaoOK = "#28a745";
                            }
                             else
                            if(ItemsSelecionados[k].ValorVerificacaoCorrecao == "Х")
                            {
                                    var el = jQuery(`<div class="tituloAV VCERRO" id="TituloAV${i}${j}${k}" style=" background-color: #c8233366" title="Verificação de Correção - Com Erros"><center>VC<center></div>`, {
                                    }).appendTo('#GridBts'+i+j+k);
                                    CorBotaoNOK = "#c82333";
                            }
                            else
                            {
                                   var el = jQuery(`<div class="tituloAV" id="TituloAV${i}${j}${k}" style=" background-color: #6c757d66" title="Verificação de Correção - Não se Aplica"><center>VC<center></div>`, {
                                }).appendTo('#GridBts'+i+j+k);
                                CorBotaoNA = "#6c757d";
                            }
                            ...
                            var el = jQuery(`
                                ...
                                <div class="grid-BTN3" style="height: 35px">
                                    <button class="btn-danger" id="BotaoNOK${i}${j}${k}" style="background-color: ${CorBotaoNOK}; color: white; width: 40.3px" type="button" onclick="AlterarValorLVE(this,'${ItemsSelecionados[k].id}', 'Х', '${i}${j}${k}', 'VerificacaoCorrecao')" title="Com Erros">Х</button>
                                    <button class="btn-success" id="BotaoOK${i}${j}${k}" type="button" style="background-color: ${CorBotaoOK}; color: white; width: 40.3px" onclick="AlterarValorLVE(this,'${ItemsSelecionados[k].id}', '✓', '${i}${j}${k}', 'VerificacaoCorrecao')" title="OK">✓</button>
                                    <button class="btn-secondary" id="BotaoNA${i}${j}${k}" type="button" style="background-color: ${CorBotaoNA}; color: white; width: 40.3px" onclick="AlterarValorLVE(this,'${ItemsSelecionados[k].id}', 'Ⱥ', '${i}${j}${k}', 'VerificacaoCorrecao')" title="Não se Aplica">N/A</button>
                                </div>`, {
                            id: 'SelectV'+i+j+k
                        }).appendTo('#GridBts'+i+j+k);
...
function CriarNovaVerificacaoCopia()
{
 var IDVerificacaoAtual = $( "#SelectDetalhesPreenchimento option:selected" ).val();
 $("#loaderVerificacao").show();
    $.ajax({
            url : "/app/civil/PreenchimentoLVEView/", // the endpoint
            type : "POST", // http method
            data : { Copiar : true, IDLveAtual : IDVerificacaoAtual}, // data sent with the post request
...
function PreencherSelectLVES(Disciplina, TipoVerifica){
    ...
    var request = $.ajax({
      ...
      method: "GET",
      data: { Disciplina : $('#IDDisciplina').html(), TipoVerifica: 'Modelo', VerificarDisc: true }
    });
...
async function PreencherTiposLVEAs(){
 ...
 let request;
      ...
      url: "/app/civil/RetornarLVEVerificacao/",
      method: "GET",
      data: {Disciplina : $('#IDDisciplina').html(), TipoVerifica: 'Modelo'  }
    });
...
function PreencherSelectVerificacoes(){
    ...
    var request = $.ajax({
      ...
      method: "GET",
      data: { id : IDAtividade, IDModelo: $("#IDModelo").html() }
    });
...
function CadastrarPreenchimento(){
    if($("#SelectLVE option:selected").text() != "---------" && $("#CadastrarPreenchimentoSelect option:selected").text() != "---------"    )
    {
        ...
        $.ajax({
            url : "/app/civil/PreenchimentoLVEView/", // the endpoint
            type : "POST", // http method
            data : { Cadastrar : true, IDLve : IDLVeSelecionadaCad, IDPessoa: ResponsavelCad, IDAtividade: IDAtividade, IDModelo: $("#IDModelo").html() }, // data sent with the post request
...
async function CheckAlteracaoStatus()
{
    ...
    if(StatusNovo == "Verificado")
    {
        ...
        {
            if(Etapas[i].Etapa == "Verificação de Modelo")
            {
                Achou = true;
                break;
            }
    ...
    if(StatusNovo == "Corrigido" )
    {
        ...
        for(var i = 0; i < Etapas.length; i++)
        {
            if(Etapas[i].Etapa == "Correção de Modelo")
            {
                Achou = true;
                break;
            }
    ...
    if(StatusNovo == "Concluido" || StatusNovo == "Correção Reprovada")
    {
        ...
        for(var i = 0; i < Etapas.length; i++)
        {
            if(Etapas[i].Etapa == "Verificação da Correção de Modelo")
            {
                Achou = true;
                break;
            }
...
var Data = $("#DataVlrForm").val();
    ...
    if(IDSelecionado != "---------")
    {
        ...
        if(CheckAlteracaoStatusEtapa == true)
        {
          ...
          $.ajax({
                url : "/app/civil/VerificacaoModelo/"+IDVerificacao+"/", // the endpoint
                type : "POST", // http method
            ...
            else
            {
                ...
                var EtapaAlterar = "";
                if(NomeStatusAlterar=="Verificado") EtapaAlterar = "Verificação de Modelo";
                else if(NomeStatusAlterar=="Corrigido") EtapaAlterar = "Correção de Modelo";
                else EtapaAlterar = "Verificação de Correção de Modelo";
...
function CriarComentarioForm() {
     $.ajax({
        ...
        type : "POST", // http method
        data : { Comentario : $('#observacao-comentario').val(), AtividadeID : $('#IDAtiv').html(), Origem: "Verificação da Correção Modelo" }, // data sent with the post request
