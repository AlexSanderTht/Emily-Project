...
function SalvaPreenchimentoLVE(ItemLVE)
{
    ...
    $.ajax({
            ...
            data : { SalvarUnitario : true, IDPreenchimentoLVE : ItemLVE.ID, Funcao: "AutoVerificacao", Valor: ItemLVE.Valor}, // data sent with the post request
...
function AlterarValorLVE(Elemento, ID, valor, NomeBotao)
{
    for(var i = 0; i < OpcoesLVEs.length; i++)
    {
        if(OpcoesLVEs[i].ID == ID)
        {
            OpcoesLVEs[i].Valor = valor;
            SalvaPreenchimentoLVE(OpcoesLVEs[i]);
            if(valor == "Х")
            {
                $("#TituloAV"+NomeBotao).css("background-color", "#c8233366");
                $("#TituloAV"+NomeBotao).css("border-color", "#c82333");
                $("#TituloAV"+NomeBotao).prop('title', 'Auto Verificação - Com Erros');
            ...
            else if(valor == "✓")
            {
                ...
                $("#TituloAV"+NomeBotao).prop('title', 'Auto Verificação - OK');
                $("#BotaoNA"+NomeBotao).css("background-color", "#6c757d66");
            ...
            else
            {
                ...
                $("#TituloAV"+NomeBotao).prop('title', 'Auto Verificação - Não se aplica');
...
OpcoesLVEs = [];
    ...
    for(var i = 0; i < Selecoes.length; i++)
    {
        ...
        jQuery('<p class="alert" style="margin-bottom: 0px !important"><strong>'+Selecoes[i]+"</strong></p>", {
            id: 'ColP'+i,
                   ...
                   if(ItemsSelecionados[k].Grupo == Grupos[j] && ItemsSelecionados[k].Selecao == Selecoes[i])
                    {
                       ...
                       else
                       {
                        ...
                        var el = jQuery('<div style="display: table-cell; vertical-align: middle;"><p class="TextoLVE " >'+ItemsSelecionados[k].Nome+"</p></div>", {
                            ...
                            {
                                   var el = jQuery(`<div class="tituloAV" id="TituloAV${i}${j}${k}" style="width: 92px; background-color: #6c757d66" title="Auto Verificação - Não Preenchido"><center>AV<center></div>`, {
                            ...
                            {
                                   var el = jQuery(`<div class="tituloAV" id="TituloAV${i}${j}${k}" style="width: 92px; background-color: #28a74566" title="Auto Verificação - OK"><center>AV<center></div>`, {
                                    }).appendTo('#GridBts'+i+j+k);
                                       CorBotaoOK = "#28a745";
                            }
                             else
                            ...
                            {
                                    var el = jQuery(`<div class="tituloAV" id="TituloAV${i}${j}${k}" style="width: 92px; background-color: #c8233366" title="Auto Verificação - Com Erros"><center>AV<center></div>`, {
                                    }).appendTo('#GridBts'+i+j+k);
                                    CorBotaoNOK = "#c82333";
                            }
                            else
                            {
                                   var el = jQuery(`<div class="tituloAV" id="TituloAV${i}${j}${k}" style="width: 92px; background-color: #cbc3ba" title="Auto Verificação - Não se Aplica"><center>AV<center></div>`, {
                                }).appendTo('#GridBts'+i+j+k);
                                CorBotaoNA = "#cbc3ba";
                            }
                            ...
                            var el = jQuery(`
                                ...
                                <div class="grid-titulo3" style="height: 35px">
                                    <button class="btn-danger" id="BotaoNOK${i}${j}${k}" style="background-color: ${CorBotaoNOK}; color: white; width: 30px" type="button" onclick="AlterarValorLVE(this,'${ItemsSelecionados[k].id}', 'Х', '${i}${j}${k}')" title="Com Erros">Х</button>
                                    <button class="btn-success" id="BotaoOK${i}${j}${k}" type="button" style="background-color: ${CorBotaoOK}; color: white; width: 30px" onclick="AlterarValorLVE(this,'${ItemsSelecionados[k].id}', '✓', '${i}${j}${k}')" title="OK">✓</button>
                                    <button class="btn-secondary" id="BotaoNA${i}${j}${k}" type="button" style="background-color: ${CorBotaoNA}; color: white; width: 30px" onclick="AlterarValorLVE(this,'${ItemsSelecionados[k].id}', 'Ⱥ', '${i}${j}${k}')" title="Não se Aplica">N/A</button>
                                </div>`, {
                            id: 'Select'+i+j+k
                        }).appendTo('#GridBts'+i+j+k);


                        OpcoesLVEs.push({Elemento: el, Selecao: Selecoes[i], Grupo: Grupos[j], LVE: ItemsSelecionados[k].Nome, ID: ItemsSelecionados[k].id, Valor: ItemsSelecionados[k].ValorAutoVerificacao });
...
{
var OpcoesSemPreencher = [];
   for(var i = 0; i < OpcoesLVEs.length; i++)
   {
        if(OpcoesLVEs[i].Valor == "")
        {
            OpcoesSemPreencher.push(OpcoesLVEs[i]);
        }
   }

   if(OpcoesSemPreencher.length > 0)
   {
        var HtmlModal = `<div class="alert alert-dark" role="alert"><strong>Os seguintes campos não foram preenchidos:</strong></div><ol>`
        for(var i = 0; i < OpcoesSemPreencher.length; i++)
        {
            HtmlModal += `<li>${OpcoesSemPreencher[i].Selecao} - ${OpcoesSemPreencher[i].Grupo} - ${OpcoesSemPreencher[i].LVE}</li>`;
        }
        HtmlModal+= `</ol><div class="alert alert-danger" role="alert"><strong>Favor preencher corretamente!</strong></div>`
        $('#Preenchimento').html(HtmlModal);
        $('#AvisoPreenchimento').modal('show')
   }
   else
   {
        $.ajax({
            ...
            type : "POST", // http method
            dataType : "json",
            data : { IDAtualizar: IDLVeSelecionada, Itens: JSON.stringify(OpcoesLVEs) }, // data sent with the post request
...
var IDSelecionado = $( "#ResponsavelModelo option:selected" ).text();
...
if(IDSelecionado != "---------")
{
    ...
    var IDModelo = $("#IDModelo").html();

      $.ajax({
            url : "/app/civil/Modelo/"+IDModelo+"/", // the endpoint
            type : "POST", // http method
            data : { IDModelo : IDModelo, IDStatus : StatusNovo, IDPessoa: IDSelecionado, DataOp: Data }, // data sent with the post request
...
function CriarComentarioForm() {
     $.ajax({
        ...
        type : "POST", // http method
        data : { Comentario : $('#observacao-comentario').val(), AtividadeID : $('#IDAtiv').html(), Origem: "Modelo" }, // data sent with the post request
