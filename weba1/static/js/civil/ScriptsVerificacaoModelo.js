...
var NomeEtapa = "Verificação de Modelo";
...
function AlterarValorLVE(Elemento, ID, valor, NomeBotao, Funcao)
{
            ...
            if(valor == "Х")
            {
                ...
                $("#TituloAV"+NomeBotao).prop('title', `Verificação - Com Erros`);
            }
            else if(valor == "✓")
            {
                ...
                $("#BotaoNOK"+NomeBotao).css("background-color", "#6c757d66");
                 $("#TituloAV"+NomeBotao).prop('title', `Verificação - OK`);
            }
            else
            {
                ...
                $("#BotaoOK"+NomeBotao).css("background-color", "#6c757d66");
                $("#BotaoNOK"+NomeBotao).css("background-color", "#6c757d66");
                 $("#TituloAV"+NomeBotao).prop('title', `Verificação - Não se Aplica`);
                       ...
                       else
                       {
                        ...
                        /*    var el = jQuery(`
                         ...
                         var HTMLHeader = "";
                            ...
                            {
                                   var el = jQuery(`<div class="tituloAV" id="TituloAV${i}${j}${k}" style="background-color: #6c757d66" title="Verificação - Não Preenchido"><center>V<center></div>`, {
                                 }).appendTo('#GridBts'+i+j+k);
                            ...
                            {
                                   var el = jQuery(`<div class="tituloAV" id="TituloAV${i}${j}${k}" style=" background-color: #28a74566" title="Verificação - OK"><center>V<center></div>`, {
                                    }).appendTo('#GridBts'+i+j+k);
                                       CorBotaoOK = "#28a745";
                            }
                             else
                            ...
                            {
                                    var el = jQuery(`<div class="tituloAV" id="TituloAV${i}${j}${k}" style=" background-color: #c8233366" title="Verificação - Com Erros"><center>V<center></div>`, {
                                    }).appendTo('#GridBts'+i+j+k);
                                    CorBotaoNOK = "#c82333";
                            }
                            else
                            {
                                   var el = jQuery(`<div class="tituloAV" id="TituloAV${i}${j}${k}" style=" background-color: #6c757d66" title="Verificação - Não se Aplica"><center>V<center></div>`, {
                                }).appendTo('#GridBts'+i+j+k);
                                CorBotaoNA = "#6c757d";
                            }
                            ...
                            var el = jQuery(`
                                ...
                                <div class="grid-titulo3" style="height: 35px">
                                    <button class="btn-danger" id="BotaoNOK${i}${j}${k}" style="background-color: ${CorBotaoNOK}; color: white; width: 30px" type="button" onclick="AlterarValorLVE(this,'${ItemsSelecionados[k].id}', 'Х', '${i}${j}${k}', 'Verificacao')" title="Com Erros">Х</button>
                                    <button class="btn-success" id="BotaoOK${i}${j}${k}" type="button" style="background-color: ${CorBotaoOK}; color: white; width: 30px" onclick="AlterarValorLVE(this,'${ItemsSelecionados[k].id}', '✓', '${i}${j}${k}', 'Verificacao')" title="OK">✓</button>
                                    <button class="btn-secondary" id="BotaoNA${i}${j}${k}" type="button" style="background-color: ${CorBotaoNA}; color: white; width: 30px" onclick="AlterarValorLVE(this,'${ItemsSelecionados[k].id}', 'Ⱥ', '${i}${j}${k}', 'Verificacao')" title="Não se Aplica">N/A</button>
                                </div>`, {
                            id: 'SelectV'+i+j+k
                        }).appendTo('#GridBts'+i+j+k);
...
$('#SelectDetalhesPreenchimento').change(function() {
   if($("#SelectDetalhesPreenchimento option:selected").text() != "---------")
    ...
    else
    {
       ...
       $('#AutoVerificacao').html(`<div class="flex"><button class="subscribe btn btn-warning  rounded-pill shadow-sm BotaoBorder flex-item" style="width: 50%; margin-top: 0px" data-toggle="modal" data-target="#CadastroPreenchimento"  data-target="#md-fade-scale">Criar Nova Verificação</button></div>`);
    }
...
function CadastrarPreenchimento(){
    if($("#SelectLVE option:selected").text() != "---------" && $("#CadastrarPreenchimentoSelect option:selected").text() != "---------"    )
    {
        ...
        $("#loaderCadastroPreenchimento").show();
        ...
        $.ajax({
            ...
            // handle a successful response
            success : function(json) {
            document.getElementById("loaderCadastroPreenchimento").style.display = "none";
             PreencherSelectVerificacoes();
                 $('#CadastroPreenchimento').modal('hide')
...
function CriarComentarioForm() {
     $.ajax({
        ...
        type : "POST", // http method
        data : { Comentario : $('#observacao-comentario').val(), AtividadeID : $('#IDAtiv').html(), Origem: "Verificação Modelo" }, // data sent with the post request
