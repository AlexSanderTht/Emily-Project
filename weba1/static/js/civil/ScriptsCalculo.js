        ...
        Grupos.sort();
         ...
         for(var j = 0; j < Grupos.length; j++){
                       jQuery('<div/>', {
                        ...
                        jQuery('<p class="alert"><strong>'+Grupos[j]+"</strong></p>", {
                            id: 'ColP'+i+j,
                            "style": 'margin: auto;'
...
function PreencherSelectLVES(Disciplina, TipoVerifica){
    ...
    var request = $.ajax({
      ...
      method: "GET",
      data: { Disciplina : $('#IDDisciplina').html(), TipoVerifica: 'Cálculo', VerificarDisc: true }
    });
...
async function PreencherTiposLVEAs(){
 ...
 let request;
      ...
      url: "/app/civil/RetornarLVEVerificacao/",
      method: "GET",
      data: {Disciplina : $('#IDDisciplina').html(), TipoVerifica: 'Cálculo'  }
    });
...
function PreencherSelectVerificacoes(){
    ...
    var request = $.ajax({
      ...
      method: "GET",
      data: { id : IDAtividade, IDCalculo: $("#IDCalculo").html() }
    });
...
function CadastrarPreenchimento(){
    if($("#SelectLVE option:selected").text() != "---------" && $("#CadastrarPreenchimentoSelect option:selected").text() != "---------"    )
    {
        ...
        $.ajax({
            url : "/app/civil/PreenchimentoLVEView/", // the endpoint
            type : "POST", // http method
            data : { Cadastrar : true, IDLve : IDLVeSelecionadaCad, IDPessoa: ResponsavelCad, IDAtividade: IDAtividade, IDCalculo: $("#IDCalculo").html() }, // data sent with the post request
...
var IDSelecionado = $( "#ResponsavelCalculo option:selected" ).text();
...
if(IDSelecionado != "---------")
{
    ...
    var IDCalculo = $("#IDCalculo").html();
      $.ajax({
            url : "/app/civil/Calculo/"+IDCalculo+"/", // the endpoint
            type : "POST", // http method
            data : { IDCalculo : IDCalculo, IDStatus : StatusNovo, IDPessoa: IDSelecionado, DataOp: Data }, // data sent with the post request
...
function CriarComentarioForm() {
     $.ajax({
        ...
        type : "POST", // http method
        data : { Comentario : $('#observacao-comentario').val(), AtividadeID : $('#IDAtiv').html(), Origem: "Cálculo" }, // data sent with the post request
